...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/FOX-GOD/ADM-WOLI/FOX-GOD-patch-1/Filescph/blockBT.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} No se puede vincular a Github !" && exit 0
	wget https://raw.githubusercontent.com/Waldo60/A-peru/main/gerador/blockBT.sh -O /etc/ger-frm/blockBT.sh &> /dev/null
...
echo -e "  Panel De Firewall VPS•BO ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}"
msg -bar2
